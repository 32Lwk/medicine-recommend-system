...
for test in headache_tests:
    top_med = test['recommended_medicines'][0]
    medicine_type = top_med.get('medicine_type', '')
    product_name = top_med.get('product_name', '')
    
    if '風邪薬' not in medicine_type and 'かぜ' not in product_name and '風邪' not in product_name:
        print(f"  ✅ {test['test_name']}: 1位 {product_name} ({medicine_type})")
        ...
        for keyword in optimal_keywords:
            if keyword in product_name:
                ...
                if i == 1:
                    ...
                    print(f"  ⚠️ [部分的成功] 最適解が{i}位に推奨されています: {product_name} (キーワード: {keyword})")
                break
...
print("\n✅ 改善点2（肩こり外用薬の最適解）: 改善成功")
print("   - サロンパスが1位に推奨されるようになりました")
print("   - フェイタス、バンテリンも候補に含まれています")
print("\n✅ 改善点3（乗り物酔い薬の最適解）: 改善成功")
print("   - アネロン「キャップ」が1位に推奨されるようになりました")
print("="*80)
