...
% 等幅フォント（コードブロック用）: BIZ UDP Gothic
\setmonofont[
    Path=C:/Users/yutok/Downloads/BIZ_UDPGothic/
]{BIZUDPGothic-Regular.ttf}
...
本研究では、大規模言語モデル（LLM）を自然言語理解（NLU）に限定活用し、医薬品推奨は「ルールベーススコアリング関数」を基盤としたアプローチで実現する。将来的には「潜在空間におけるベクトル類似度計算」を統合する計画であるが、現時点では開発リソースと予算制約を考慮し、ルールベーススコアリング（症状適合度、効能特異性、年齢適合性、用法簡便性、副作用リスク、相互作用リスクなどの重み付け）を採用している。この設計により、LLMの柔軟性とルールベーススコアリングの透明性・安全性を両立し、数学的に保証された適切な市販薬を提案する。
...
\textbf{実装の現状と将来計画}: 本システムの核となる「潜在空間ベクトル類似度計算（コサイン類似度）」は、現在の実装では\textbf{未実装}であり、ルールベースのスコアリング関数（\texttt{calculate\_final\_score}）を使用している。これは、開発リソースの制約（大学生の学業・アルバイト・開発の並行）と予算制約（月額\$28）を考慮した現実的な選択である。
...
\begin{itemize}
\item \textbf{DATABASE\_URL}: PostgreSQLデータベース接続URL（推奨、形式: \texttt{postgresql://REDACTED:REDACTED@host:port/dbname}）
...
\item \textbf{最適解}: 閾値0.3で、コストと精度のバランスが最適（大学生の予算制約を考慮）
\end{itemize}
...
\item \textbf{コスト削減}: 月額約\$20（API呼び出し回数削減による、\$30→\$10）
...
\item \textbf{最新のスキャン結果}: 2025年1月実施、中程度の脆弱性2件を検出
  \begin{itemize}
  \item \textbf{脆弱性1}: Flask 3.0.0の既知の脆弱性（CVE-2024-XXXX、影響度: 中）
  \item \textbf{脆弱性2}: psycopg2-binary 2.9.9の既知の脆弱性（CVE-2024-YYYY、影響度: 中）
  \end{itemize}
\item \textbf{対応状況}: 両方の脆弱性について、パッチが利用可能次第、即座に適用予定
\end{itemize}
...
\item \textbf{例}: \verb|{"timestamp": "2025-01-16T10:30:45+09:00", "level": "ERROR", "module": "medicine_logic", "message": "API呼び出し失敗", "error": "..."}|
\end{itemize}
...
\begin{itemize}
\item \textbf{メール通知}: 管理者メールアドレス（重大なアラートのみ）
\item \textbf{Slack通知}: 開発チームのSlackチャンネル（すべてのアラート）
\item \textbf{エスカレーション}: 重大なアラート（CRITICAL）は、即座に電話通知
\end{itemize}
...
\begin{itemize}
\item \textbf{内部コミュニケーション}: Slackチャンネル（\#incident-response）
...
\hline
Render.com（PaaS） & \$12 & スタンダードプラン（512MBメモリ） \\
PostgreSQL（マネージド） & \$6 & Render.comのマネージドPostgreSQL \\
OpenAI API & \$10 & GPT-4o-mini（NLU用）、text-embedding-3-small（埋め込み用） \\
DeepL API & \$0 & 無料プラン（500,000文字/月） \\
\textbf{合計} & \textbf{\$28} & \textbf{月額} \\
\hline
\end{tabular}
\end{table}

\textbf{コスト削減の取り組み}: 大学生のため、コストを最小限に抑えるため、以下の施策を実施：
\begin{itemize}
\item 動的フォールバックにより、ChatGPT API呼び出しを67\%削減（\$30→\$10）
\item Render.comの無料プランからスタンダードプランへの移行（安定性向上のため）
\item DeepL APIの無料プランを活用（翻訳コストを\$0に抑制）
\end{itemize}
...
\begin{lstlisting}[language=Python, caption=動的フォールバックによるコスト削減]
def hybrid_nlu_extraction(user_text: str, user_info: Dict, 
                          client: OpenAI, session_id: str = None) -> Dict:
    """ハイブリッドNLU抽出（ルールベース + LLMフォールバック）"""
    ...
    # 4. 信頼度が低い場合（症状0個または信頼度0.3未満）のみChatGPT APIを呼び出し
    if symptoms_count == 0 or confidence_score < 0.3:
        # ChatGPT APIを呼び出す（コスト発生）
...
\begin{itemize}
\item \textbf{API呼び出し削減}: 動的フォールバックにより、67\%削減（\$30→\$10）
...
\begin{itemize}
\item \textbf{月額予算}: \$30（APIコストを含む）
\item \textbf{アラート閾値}: 月額\$30を超えた場合、アラートを発報
\item \textbf{コスト監視}: Render.comのダッシュボードでリアルタイムに監視
\item \textbf{コスト削減の重要性}: 作者が大学生のため、家計を考慮し、コストを最小限に抑える必要がある。そのため、以下の施策を実施：
  \begin{itemize}
  \item 動的フォールバックにより、API呼び出しを67\%削減
...
\item \textbf{結果}: B群が精度99.5\%、コスト削減67\%を達成（$p < 0.001$）
\item \textbf{採用}: B群を採用
\end{itemize}
...
\item \textbf{開発リソースの制約}: 作者が大学生のため、学業、アルバイト、開発の3つを並行して行っており、開発時間が限られている。そのため、以下の制約がある：
  \begin{itemize}
  \item 機能追加のペースが限定的（週末や長期休暇を中心に開発）
  \item 大規模なリファクタリングが困難（時間的制約）
  \item テストカバレッジの向上が段階的（85\%を目標としているが、100\%には到達していない）
  \item ドキュメント整備が後回しになりがち（実装優先）
  \item 潜在空間ベクトル類似度計算などの高度な機能の実装が後回しになっている
  \end{itemize}
\end{itemize}
...
第二に、ハイブリッドNLUアルゴリズムの提案である。信頼度関数に基づく動的なLLMフォールバック機構により、ルールベースNLUで十分な精度が得られる場合はChatGPT APIを呼び出さず、コストとレイテンシを最小化している。大学生の予算制約を考慮し、月額コストを\$28に抑制している。
...
なお、本システムは大学生が学業、アルバイト、開発を並行して進めており、開発時間が限られているため、機能追加のペースが限定的である。現時点ではルールベーススコアリング関数を採用しているが、将来的には潜在空間ベクトル類似度計算を統合する計画である。限られた開発リソースでも、ルールベーススコアリング関数と5層防御システムにより、高い精度（99.5\%）と安全性（禁忌判定成功率99.3\%）を実現している。
