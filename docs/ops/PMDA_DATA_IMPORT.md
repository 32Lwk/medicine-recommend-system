...
| 項目 | 値 |
|------|-----|
| リクエスト間隔 | **2.5–5.0 秒**（uniform jitter、`--min-interval` 下限。本番は `1` 可） |
| 1 セッション上限 | **30 HTTP**（`--live-batch-size 30`）。成分 batch は **10** |
| abort 条件 | 403 / 429×3 / 連続 empty HTML×3 |
| 429 バックオフ | 60s → 120s → 300s 後 abort |
| セッション間隔 | **最低 4 時間**（`last_session_end_at`） |
| abort 後 | **24 時間**再試行禁止 |
| User-Agent | 固定 `medicine-recommend-pmda-import/1.0`（偽装禁止） |
| 実行場所 | **ローカル回線のみ**（CodePipeline / AWS IP 禁止） |
| 時間帯 | **推奨: JST 22:00–06:00**。昼間は `--allow-daytime` + **1 日最大 2 セッション**（`manifest.live_fetch.sessions_today`） |
...
| 指標 | Before (baseline) | After (Phase 2) |
|------|-------------------|-----------------|
| score_pass | 65% (13/20) | **80% (16/20)** |
| pass_all | 60% (12/20) | **80% (16/20)** |
| ingestion job | — | `K4XOSLSFXI` COMPLETE（19,603 indexed） |

失敗 4 問（score 未達）: `usage-water-amount`, `doping-pseudoephedrine`, `doping-prohibited-substance`, `age-elderly-nsaid`  
→ いずれも **catalog expansion 行が原因ではない**（products 検索のスコア不足・汎用ドーピング問いの 0 ヒット）。
