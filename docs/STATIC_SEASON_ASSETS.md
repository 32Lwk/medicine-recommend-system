...
- **行事専用ディレクトリ**: 七夕・敬老・ハロウィン・七五三などは `static/img/events/<行事名>/` にまとめ、`base_path` を `events/<行事名>` とする（淡色 PNG をリポジトリにコミット。生成記録は `docs/PARTICLE_AI_SPRITES.md` の「左右装飾」節。任意のプレースホルダ再生成は `scripts/gen_event_decoration_pngs.py` でも可）。
