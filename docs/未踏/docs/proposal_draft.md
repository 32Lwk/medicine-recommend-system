...
**図1（既存図の流用）**: 下記のいずれかを挿入する。  
- **システム構成**: `docs/LaTex/photo/system.png`  
- **データフロー（推奨）**: `docs/LaTex/photo/dataflow_detailed.png` または `dataflow_detailed.pdf`（ルールベース＋潜在空間の流れが分かる）  
- **5層防御のイメージ**: `docs/LaTex/photo/defense_layers.png` または `defense_layers.pdf`  

※ 提出用資料では、上記のいずれか（または2つを並べて）をコピーし、図のキャプションを「図1: システム構成（5段階パイプライン：リスク→NLU→禁忌→スコアリング→上位3件推奨）。LLMはNLUに限定。」などとして使用する。
...
**図2（開発線表）**: ガントチャートは次のファイルで用意した。提出用資料には、画像として挿入すること。  
- **Mermaid ソース**: `docs/未踏/docs/gantt_schedule.mmd`  
- **画像の作り方**: `docs/未踏/docs/gantt_schedule.html` をブラウザで開き、表示された図を右クリック→「画像を別名で保存」で PNG 保存するか、印刷→PDFで保存する。  
- **オンライン**: [Mermaid Live Editor](https://mermaid.live) に `gantt_schedule.mmd` の内容を貼り付けて PNG/SVG をエクスポートしてもよい。
...
- **図の挿入（既存図の流用）**  
  - **図1（① 何を作るか）**: `docs/LaTex/photo/` の既存図を流用する。  
    - システム全体: `system.png`  
    - データフロー（推奨）: `dataflow_detailed.png` または `dataflow_detailed.pdf`  
    - 5層防御: `defense_layers.png` または `defense_layers.pdf`  
  - **図2（④ 開発線表）**: `docs/未踏/docs/gantt_schedule.html` をブラウザで開いて画像を保存するか、`gantt_schedule.mmd` を Mermaid Live Editor で開いて PNG/SVG をエクスポートし、④(6) の開発線表として挿入する。  
  - いずれも A4・8ページ程度に収まるようサイズを調整して配置すること。
