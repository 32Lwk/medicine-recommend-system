...
gcloud run deploy "$SERVICE" \
  ...
  --cpu=1 \
  --no-cpu-throttling \
  --update-env-vars "CHAT_WORKER_MAX=8,CHAT_STREAM_TIMEOUT_SEC=180,GUNICORN_WORKER_CLASS=uvicorn.workers.UvicornWorker,GUNICORN_WORKERS=2,EXPLAIN_BATCH_HARD_TIMEOUT_SEC=45,MEDICINE_KB_AUGMENT_TIMEOUT_SEC=15,PROCESSING_STATUS_READ_CACHE_SEC=2.5,OPENAI_MODEL_EXPLAIN=gpt-4o-mini,LATENCY_EXPLAIN_FAST_LOWRISK=1,MEDICINE_RAG_PROVIDER=none" \
  --quiet
