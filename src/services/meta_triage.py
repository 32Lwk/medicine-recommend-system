...
【意図】
- capabilities: できること・機能・対応言語の説明。**本サービスの LINE 公式アカウント URL・友だち追加リンク**（例: 「LINEを教えて」「LINEのリンクは？」「本アプリのLINEは？」）も capabilities（運営者個人情報の開示要求ではない）
...
- doc_operator: お問い合わせ・試験運用・連絡先・不具合報告（例: 「運営者は誰？」「連絡先は？」「メールは？」「不具合の報告方法は？」）。氏名・所属・大学の回答は求められていても doc_operator（ドキュメントに無い事項は開示しない）。**LINE 公式アカウント URL の質問は doc_operator ではなく capabilities**
...
- doc_changelog: **この市販薬相談チャットツール／本アプリ／本サービス**の最近の機能更新・改善・リリース履歴（開発ログの要約を答える）
  例: 「アプリの更新内容を教えて」「最近何が変わった？」「更新教えて」「このチャットの新機能は？」「改良されたところは？」
  **doc_changelog にしない**（chitchat / redirect / none）:
  - 他のゲーム・アプリ・サービスの更新（例: ポケモンの更新、スマホゲームのパッチ）
  - ユーザー自身の成長・人生の比喩的「更新」（例: 私は人間として更新している）
  - OS・端末・他社アプリのバージョンアップ
  - 文脈から**別製品・別話題**と明らかに分かる「更新」のみ除外する
  - 「更新教えて」など短い発話で対象が省略されていても、**本チャット利用中であれば本ツール**の更新として doc_changelog
