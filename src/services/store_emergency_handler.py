...
MEDICAL_TERMS_WITH_EN = [
    ...
    '結膜炎', '咽頭炎', '喉頭炎', '扁桃炎', '副鼻腔炎', '鼻炎', '鼻アレルギー', '中耳炎', '外耳炎',
    ...
    for emergency_type, keywords in EMERGENCY_KEYWORDS.items():
        for keyword in keywords:
            if keyword in user_text_lower:
                ...
                # 3.5 短い窃盗キーワード「スリ」が医療用語（インスリン等）の部分一致
                if not should_exclude and keyword == "スリ":
                    if "インスリン" in user_text or "インスリン" in context:
                        should_exclude = True
                        logger.info("🔍 医療用語インスリン内の部分一致として除外: スリ")
