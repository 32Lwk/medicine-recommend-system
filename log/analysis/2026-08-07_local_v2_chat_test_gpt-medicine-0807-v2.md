...
- **medicine_thread**: 0/4 自動合格 / 0 ターン
...
| **shadow_regression_mismatch_rate_pct** | **2.55%** (43/1686) |
| shadow_mismatch_rate_pct | 12.1% |
| shadow_improvement_mismatch_rate_pct | 8.54% |
| shadow_exempt_rate_pct | 1.01% |
| dispatch_unhandled | 23 |
| shadow_by_mismatch_kind | agree:1482, exempt:17, gate_improvement:144, regression:43 |

## カテゴリ別

| カテゴリ | セッション | ターン | 合格 | 要確認 |
|----------|------------|--------|------|--------|
| medicine_thread | 4 | 0 | 0 | 4 |

## レイテンシ（KPI: p95 < 5s）

- 計測ターン数: 0
- end-to-end: p50 0.0ms / **p95 0.0ms** / max 0.0ms
...
|------------|----------|-------|------------|--------------|------------|
| `` | gpt-medicine-thread-loxonin-casual | 0 | 0/0 | 0 | — |
| `` | gpt-medicine-thread-multi-compare | 0 | 0/0 | 0 | — |
| `` | gpt-medicine-thread-elderly-vague | 0 | 0/0 | 0 | — |
| `` | gpt-medicine-thread-young-slang | 0 | 0/0 | 0 | — |

## 自動メトリクス（gcp-log-analysis 系）

```json
{
  "since_unix": 1786048665.4965477,
  "pipeline_baseline": {
    "exit_code": 0,
    "data": {
      "counseling_detail_path": "D:\\Programing\\medicine-recommend\\log\\counseling_detail_log.jsonl",
      "counseling_detail_total": 10164,
      "with_response": 10164,
      "response_missing": 0,
      ...
      "latency": {
        ...
        "llm_by_path": {
          ...
          "medicine_qa/focus_llm": {
            "count": 2364,
            "latency_ms_sum": 2702318.39,
            "latency_ms_p50": 1047.46,
            "latency_ms_p95": 1734.93
          },
          "llm_triage.stage1": {
            "count": 1366,
            "latency_ms_sum": 2078074.42,
...
|----|----------|------------|----------|-----------|
| gpt-medicine-thread-loxonin-casual | medicine_thread | `` | exception:('Connection aborted.', ConnectionResetError(10054, '既存の接続はリモート ホストに強制的に切断されました。', None, 1 |  |
| gpt-medicine-thread-multi-compare | medicine_thread | `` | exception:HTTPConnectionPool(host='127.0.0.1', port=5000): Max retries exceeded with url: /new_sessi |  |
| gpt-medicine-thread-elderly-vague | medicine_thread | `` | exception:HTTPConnectionPool(host='127.0.0.1', port=5000): Max retries exceeded with url: /new_sessi |  |
| gpt-medicine-thread-young-slang | medicine_thread | `` | exception:('Connection aborted.', ConnectionResetError(10054, '既存の接続はリモート ホストに強制的に切断されました。', None, 1 |  |

## 全セッション — 完全トランスクリプト

### gpt-medicine-thread-loxonin-casual — medicine_thread (REVIEW)
- session_id: ``
- wave: gpt-scale
- persona: medicine-thread-loxonin-casual
### gpt-medicine-thread-multi-compare — medicine_thread (REVIEW)
- session_id: ``
- wave: gpt-scale
- persona: medicine-thread-multi-compare
### gpt-medicine-thread-elderly-vague — medicine_thread (REVIEW)
- session_id: ``
- wave: gpt-scale
- persona: medicine-thread-elderly-vague
### gpt-medicine-thread-young-slang — medicine_thread (REVIEW)
- session_id: ``
- wave: gpt-scale
