...
## ☁️ クラウド移行・本番環境（2026年2月）

本システムは **2026年2月** に Render + Render PostgreSQL から **GCP Cloud Run + Neon PostgreSQL** へ移行しました。移行期間は **2日間** です。

### 移行の概要

| 項目 | 移行前 | 移行後 |
|------|--------|--------|
| **アプリケーション** | Render（Web Service） | **GCP Cloud Run**（コンテナ・サーバーレス） |
| **データベース** | Render PostgreSQL | **Neon PostgreSQL**（サーバーレス） |
| **CI/CD** | Render 自動デプロイ | **Cloud Build**（GitHub push で自動ビルド・デプロイ） |
| **リージョン** | 米国など | **asia-northeast1（東京）** |

### 技術的な変更点

| 変更内容 | 詳細 |
|----------|------|
| **Docker 化** | `Dockerfile` を新規作成。Python 3.11、Gunicorn 起動。Cloud Run の `PORT=8080` に対応。 |
| **接続文字列** | `DATABASE_URL` を Neon の接続形式（`postgresql://...@ep-xxx.neon.tech/neondb?sslmode=require`）に変更。 |
| **Cloud SQL 接続の削除** | Neon 利用に伴い、Cloud Run の「Cloud SQL 接続」設定は不要（削除済み）。 |
| **環境変数** | `OPENAI_API_KEY`、`DEEPL_API_KEY`、`SECRET_KEY`、`DATABASE_URL` を Cloud Run の環境変数で設定。 |

### 移行による利点

| 利点 | 説明 |
|------|------|
| **コスト削減** | Cloud Run はリクエスト時のみ課金（スケールゼロ）。Neon は無料枠〜低価格の従量課金で、月額を大幅に抑制。 |
| **スケーラビリティ** | アクセスに応じて自動スケール。トラフィック増加時も手動設定なしで対応可能。 |
| **継続的デプロイ** | GitHub の `main` ブランチへ push すると、Cloud Build が自動でビルド・デプロイ。 |
| **東京リージョン** | 日本ユーザー向けにレイテンシを低減。 |
| **長時間リクエスト対応** | リクエストタイムアウト 300 秒に対応（本アプリの推奨処理 25–30 秒に余裕で対応）。 |

### 拡張性の広がり

- **コンテナベース**: 一度 Docker 化しているため、GKE や他クラウドへの移行が容易。
- **サーバーレス**: インフラ管理の負担が少なく、開発に集中しやすい。
- **マルチリージョン**: 将来的に他リージョンへデプロイしてレイテンシ最適化が可能。
- **Neon のスケールゼロ**: DB もアイドル時は課金が抑えられ、トラフィックに応じてスケール。

### コスト面の変化（概算）

| 構成 | 月額目安 |
|------|----------|
| **移行前（Render + Render PostgreSQL）** | 約 $25 |
| **移行後（Cloud Run + Neon）** | 約 ¥0〜数百円（Neon 無料枠・Cloud Run 従量課金） |

※ 移行前は Render で約 $25/月。移行後は Neon を無料枠で運用し、Cloud Run はリクエスト・処理時間に応じた従量課金のため、トラフィックが少ない場合は月額ほぼゼロで運用可能。
...
- カスタムドメインの設定も可能（Cloud Run → ドメインのマッピング）。

### 移行計画の詳細

移行手順の詳細は `docs/GCP移行計画.md` を参照してください。
...
- `DATABASE_URL`: PostgreSQLデータベース接続URL（推奨）
  ...
  - 形式: `postgresql://username:password@host:port/database_name`
  - 例: `postgresql://user:pass@localhost:5432/medicine_feedback`
...
OPENAI_API_KEY=your-api-key-here
DEEPL_API_KEY=your-deepl-api-key
DATABASE_URL=postgresql://username:password@localhost:5432/medicine_feedback
...
export DATABASE_URL="postgresql://username:password@localhost:5432/medicine_feedback"
export SECRET_KEY="your-secret-key-here"

# Windows PowerShell
[Environment]::SetEnvironmentVariable("OPENAI_API_KEY", "your-api-key-here", "User")
[Environment]::SetEnvironmentVariable("DEEPL_API_KEY", "your-deepl-api-key", "User")
[Environment]::SetEnvironmentVariable("DATABASE_URL", "postgresql://username:password@localhost:5432/medicine_feedback", "User")
