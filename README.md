...
**最終更新日: 2025年12月31日（診断名検出機能の大幅改善・ユーザーメッセージ表示の修正・イースターエッグ機能の大幅拡張・成分重複チェック機能・曖昧入力検出の改善・総合感冒薬推奨ロジックの強化・カテゴリ多様性の確保・アドバイス生成の改善・管理画面UI改善・薬剤師要請機能の改善）**
...
- **ログ記録**: イースターエッグ発動時のログを記録（デバッグ・分析用）
  ...
  - エラー発生時も詳細なログを記録
- **実装ファイル**:
  - `static/js/easter-eggs.js`: イースターエッグ検出・実行ロジック（Unicode 16.0 / Emoji 16.1準拠の絵文字パーティクル効果、拡張された画面変形機能を含む）
  - `static/js/games/snake.js`: スネークゲーム実装
  - `static/css/easter-eggs.css`: イースターエッグ専用CSS（画面変形、モーダル、アニメーション）
...
- **特別イベント系イースターエッグ（2025年12月31日追加）**: 13種類の特別イベントに対応したパーティクル効果とお祝いメッセージ
  - **新年（New Year）**: 「あけましておめでとう」「良いお年を」「謹賀新年」「ことより」「あけおめ」など、年賀状用語（賀正、迎春、初春）を含む多数のトリガーに対応。花火アニメーション + 新年パーティクル（🎊、🎉、🎈、✨、⭐、🌟、💫、🎁）が表示されます
  ...
  - **大晦日（New Year's Eve）**: 「大晦日」「年越し」「良いお年を」などに対応。大晦日パーティクル（🎊、🎉、🎈、✨、⭐、🌟、💫、🎁）が表示されます
...
- **イースターエッグ機能の大幅拡張（特別イベント系の追加）**
  ...
  - **大晦日イースターエッグ**: 「大晦日」「年越し」「良いお年を」などに対応。大晦日パーティクル（🎊、🎉、🎈、✨、⭐、🌟、💫、🎁）が表示されます
  - **トリガーキーワードの大幅拡充**: 各イベントに対して、敬語形・カジュアル形、ひらがな・カタカナ・漢字のバリエーション、英語のバリエーション（wishes、greetingsなど）、関連表現を含む多数のトリガーに対応
