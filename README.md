...
### 💬 カウンセリング機能（2025年12月16日追加）
- **感情的症状への対応**: 緊張、不安、恋愛の悩みなどに対するカウンセリング的返信
- **共感的な返信生成**: ChatGPTを使用した100文字以内の簡潔な返信
...
- **カウンセリング機能の実装**
  ...
  - **共感的な返信生成**: ChatGPTを使用した100文字以内の簡潔な返信を生成
  - **フォローアップ質問**: 症状の詳細を把握するための自然な質問を自動生成
  - **話題転換の自動検知**: カウンセリング中に新しい症状が検出された場合、自動的に話題を転換
...
- **UI/UX改善**
  ...
  - **カウンセリング応答の文字数制限**: ChatGPTのカウンセリング応答を100文字に制限し、簡潔な返信を実現

- **新規モジュールの追加**
  - `llm_triage.py`: LLMトリアージ機能を実装
  - `counseling_response.py`: カウンセリング応答機能を実装
