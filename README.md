...
本リポジトリのソースコードは [PolyForm Noncommercial License 1.0.0](LICENSE)（非商用）の下で提供されています。

- **許可**: 個人利用、研究、教育、非営利団体による利用、改変・再配布（非商用の範囲）
- **禁止**: 金銭的報酬や営利を主目的とする商用利用（事前の書面による許可が必要）
- **サービス利用**: デプロイ済みアプリの利用は [免責事項・利用規約](docs/public/免責事項・利用規約.md) に従います（コードライセンスとは別）
- **対象外**: 画像素材（[アプリ概要](docs/public/アプリ概要.md) 参照）および医薬品データは、それぞれ別の権利・利用条件に従います

商用利用の相談: [運営者情報](docs/public/運営者情報.md) の連絡先
