            ...
            content: {
                ja: `
                    <div class="info-section">
                        ...
                        <ol>
                            ...
                            <li>本アプリは、医薬品医療機器等法（薬機法）および景品表示法（景表法）等の関連法令を踏まえ、診断・処方・治療行為を行わない一般用医薬品（OTC）の参考案内に限定します。</li>
                            <li>市販薬の候補提示はルールベースのアルゴリズムにより行い、チャット上の説明文はAIにより生成されます。薬効・安全性の最終判断は、医師・薬剤師・登録販売者等の専門家にご相談ください。</li>
                            <li>本アプリは試験運用（β版）であり、法令上の適合性や表示の妥当性について、運営者が保証するものではありません。</li>
                        </ol>
                ...
                en: `
                    <div class="info-section">
                        ...
                        <h4>Article 3 (Disclaimer and Compliance with Pharmaceutical Laws)</h4>
                        <ol>
                            ...
                            <li>This app is limited to providing reference information on over-the-counter (OTC) medicines without performing diagnosis, prescription, or treatment, in accordance with relevant laws including the Act on Securing Quality, Efficacy and Safety of Products Including Pharmaceuticals and Medical Devices (PMD Act) and the Act against Unjustifiable Premiums and Misleading Representations.</li>
                            <li>OTC medicine candidates are selected by a rule-based algorithm; explanations in the chat are generated by AI. For final judgments on efficacy and safety, please consult physicians, pharmacists, registered sellers, or other qualified professionals.</li>
                            <li>As a beta trial, the operator does not guarantee legal compliance or the appropriateness of representations.</li>
                        </ol>
                ...
                ko: `
                    <div class="info-section">
                        ...
                        <ol>
                            ...
                            <li>본 앱은 의약품·의료기기 등에 관한 법률(약기법) 및 표시·광고에 관한 법령(표시법) 등 관련 법령을 고려하여, 진단·처방·치료 행위를 하지 않는 일반의약품(OTC) 참고 안내에 한정합니다.</li>
                            <li>일반의약품 후보 제시는 규칙 기반 알고리즘으로 이루어지며, 채팅의 설명 문구는 AI가 생성합니다. 약효·안전성에 대한 최종 판단은 의사·약사·등록판매자 등 전문가와 상담하시기 바랍니다.</li>
                            <li>본 앱은 시험 운용(β판) 단계이며, 법령상 적합성이나 표시의 타당성에 대해 운영자가 보증하지 않습니다.</li>
                        </ol>
                ...
                zh: `
                    <div class="info-section">
                        ...
                        <ol>
                            ...
                            <li>本应用依据《药品医疗器械等相关法律》（药机法）以及《不当赠品及不当表示防止法》（景表法）等相关法令，限于不提供诊断·处方·治疗的一般用药品（OTC）参考指引。</li>
                            <li>非处方药候选由基于规则的算法选定，聊天中的说明文由AI生成。关于药效与安全性的最终判断，请咨询医师、药师、注册销售员等专业人士。</li>
                            <li>本应用处于试验运营（β版）阶段，运营者不保证法令上的合规性或表示的妥当性。</li>
                        </ol>
    ...
    function updateModalListItems() {
        ...
        items.forEach(item => {
            ...
            if (isSageUi()) {
                const sageModal = {
                    ja: {
                        'site-about': ['詳しい説明', 'ウェブ上の説明サイトへ'],
                        'app-overview': ['アプリ概要', 'β版・研究目的の説明'],
                        'usage': ['使い方', 'チャット・音声入力・薬剤師要請'],
                        'disclaimer': ['利用規約・免責', 'β版の利用条件と免責事項'],
                        'privacy': ['プライバシー', '個人情報の取り扱い'],
                        'settings': ['表示設定', '文字サイズ・カラー・演出'],
                    },
                    en: {
                        'site-about': ['More information', 'Open the web about site'],
                        'app-overview': ['App overview', 'Beta research purpose'],
                        'usage': ['How to use', 'Chat, voice input, pharmacist'],
                        'disclaimer': ['Terms & disclaimer', 'Beta terms and limitations'],
                        'privacy': ['Privacy', 'How we handle personal data'],
                        'settings': ['Display', 'Text size, colors, effects'],
                    },
                    ko: {
                        'site-about': ['자세한 설명', '웹 설명 사이트로 이동'],
                        'app-overview': ['앱 개요', 'β판·연구 목적 설명'],
                        'usage': ['사용 방법', '채팅·음성 입력·약사 요청'],
                        'disclaimer': ['이용약관·면책', 'β판 이용 조건과 면책'],
                        'privacy': ['프라이버시', '개인정보 취급'],
                        'settings': ['표시 설정', '글자 크기·색·효과'],
                    },
                    zh: {
                        'site-about': ['详细说明', '前往网页说明站点'],
                        'app-overview': ['应用概述', 'β版·研究目的说明'],
                        'usage': ['使用方法', '聊天·语音输入·药师请求'],
                        'disclaimer': ['使用条款·免责', 'β版使用条件与免责'],
                        'privacy': ['隐私', '个人信息处理'],
                        'settings': ['显示设置', '字号·颜色·效果'],
                    },
                };
                const pack = sageModal[currentLanguage] || sageModal.ja;
                if (pack[item.id]) {
                    title = pack[item.id][0];
                    desc = pack[item.id][1];
                }
            }
    ...
    /** リロード後など tab ID が変わったときの stale ロック解除 */
    function forceReleaseChatSubmitLock(sid) {
        const id = (sid || getSidFromCookie() || '').trim();
        if (!id) {
            return;
        }
        try {
            localStorage.removeItem(CHAT_SUBMIT_LOCK_PREFIX + id);
        } catch (e) { /* ignore */ }
    }
    ...
    function forceReleaseChatStreamLock(sid) {
        const id = (sid || getSidFromCookie() || '').trim();
        if (!id) {
            return;
        }
        try {
            localStorage.removeItem(CHAT_STREAM_LOCK_PREFIX + id);
        } catch (e) { /* ignore */ }
    }
    ...
    function isPostResponseReady(messages) {
        ...
        if (processingBubbleLocked || isProcessingInFlight()) {
            // セッション上ターン確定後は applyBotResponseSession で DOM 反映する（DOM 待ちだとデッドロック）
            return isCurrentTurnComplete(merged);
        }
        ...
        if (pendingUser) {
            ...
            for (let i = turn.length - 1; i >= 0; i--) {
                ...
                if (
                    message
                    && message.type === 'user'
                    && userMessagesMatch(message, pendingUser)
                ) {
                    userIndex = i;
                    break;
                }
    ...
    function mergeInFlightPendingUser(messages, sid) {
        ...
        for (let i = tail.length - 1; i >= 0; i--) {
            ...
            if (
                comparableUserMessageText(m) !== comparableUserMessageText(userText)
    ...
    function markPostResponseResolved(options) {
        ...
        forceReleaseChatSubmitLock(sid);
        forceReleaseChatStreamLock(sid);
    ...
    function shouldSuppressPostFetchError() {
        ...
        return isResponseVisibleInDom(null);
    }

    function buildMergedMessagesFromDonePayload(donePayload) {
        ...
        const merged = normalizeTurnTail(
            cached,
            donePayload.bot_message,
            donePayload.user_message,
            lastUser
        );
        return isChatResponseComplete(merged) ? merged : null;
    }

    function renderDonePayloadImmediately(donePayload) {
        const merged = buildMergedMessagesFromDonePayload(donePayload);
        if (!merged) {
            return false;
        }
        const sid = getSidFromCookie();
    ...
    chatFormEl.addEventListener('submit', function(e) {
        try {
            ...
            let submitLockAcquired = tryAcquireChatSubmitLock(submitSid);
            if (
                !submitLockAcquired
                && !loadInFlightTurn(submitSid)
                && !processingBubbleLocked
            ) {
                forceReleaseChatSubmitLock(submitSid);
                submitLockAcquired = tryAcquireChatSubmitLock(submitSid);
            }
    ...
    function restoreInFlightTurnOnPageLoad(sid) {
        ...
        const inflightUserInTail = tail.some(function (m) {
            ...
            if (!userMessagesMatch(m, userText) && !isBlockedUserPlaceholderMessage(m)) {
                return false;
            }
            if (turnId && m.pending_turn_id && m.pending_turn_id !== turnId) {
                return false;
        ...
        activeSubmitSid = sid;
        scheduleDeferredSessionRecovery(chatSubmitGeneration, 0, { maxAttempts: 30, force: true });
        return true;
    }
    ...
    function findPriorBotForUserText(messages, userText) {
        ...
        for (let i = list.length - 2; i >= 0; i--) {
            ...
            if (
                userMsg
                && userMsg.type === 'user'
                && userMessagesMatch(userMsg, text)
                && botMsg
                && botMsg.type === 'bot'
                && !botMsg.error
            ) {
    ...
    function findUserMessageByText(messages, text) {
        ...
        for (let i = list.length - 1; i >= 0; i--) {
            ...
            if (message && message.type === 'user' && userMessagesMatch(message, target)) {
                return message;
            }
        }
        return null;
    ...
    function stableMessageKey(message) {
        ...
        if (message && message.admin_request) {
            return 'admin:' + ts + ':' + content;
        }
    ...
    /** 重複判定用（\\r\\n や連続空白を正規化） */
    function comparableUserMessageText(messageOrText) {
        const raw = typeof messageOrText === 'object' && messageOrText
            ? userMessageText(messageOrText)
            : String(messageOrText || '').trim();
        return raw.replace(/\r\n?/g, ' ').replace(/\s+/g, ' ').trim();
    }

    function userMessagesMatch(a, b) {
        return comparableUserMessageText(a) === comparableUserMessageText(b);
    }

    /** 今回ターン（baseline 以降）に同一文言の user がサーバー側にあれば true */
    function serverHasUserMatchingTextInCurrentTurn(serverMsgs, text) {
        const target = comparableUserMessageText(text);
        if (!target) {
            return false;
        }
        const baseline = getChatSubmitBaselineLength();
        const slice = (Array.isArray(serverMsgs) ? serverMsgs : []).slice(baseline);
        return slice.some(function (m) {
            return m && m.type === 'user' && comparableUserMessageText(m) === target;
        });
    }

    /** 楽観 pending user がサーバー保存済み user と重複しているか */
    function isRedundantOptimisticUser(serverMsgs, localMsg) {
        if (!localMsg || localMsg.type !== 'user' || localMsg.uuid || localMsg.message_id) {
            return false;
        }
        if (!localMsg.pending_local && !localMsg.pending_turn_id && !isInflightPendingUserMessage(localMsg)) {
            return false;
        }
        return serverHasUserMatchingTextInCurrentTurn(serverMsgs, localMsg);
    }

    /** サーバー確定 user と同一文言の楽観 pending user を除去 */
    function stripRedundantOptimisticUsers(messages) {
        const list = Array.isArray(messages) ? messages : [];
        const out = [];
        list.forEach(function (m, index) {
            ...
            if (
                ...
                && !m.uuid
                && !m.message_id
                && (m.pending_local || m.pending_turn_id)
            ) {
                const text = comparableUserMessageText(m);
                const hasConfirmedDuplicate = list.some(function (other, j) {
                    if (j === index || !other || other.type !== 'user') {
                        return false;
                    }
                    if (!(other.uuid || other.message_id)) {
                        return false;
                    }
                    return comparableUserMessageText(other) === text;
                });
                if (hasConfirmedDuplicate) {
                    return;
                }
                const hasPendingDuplicateInOut = out.some(function (prev) {
                    return prev.type === 'user' && comparableUserMessageText(prev) === text;
                });
                if (hasPendingDuplicateInOut) {
                    return;
                }
            }
            out.push(m);
    ...
    function findUserMessageNodeByComparableText(chatMessages, text) {
        if (!chatMessages) {
            return null;
        }
        const target = comparableUserMessageText(text);
        ...
        for (let i = nodes.length - 1; i >= 0; i--) {
            ...
            if (comparableUserMessageText(messageNodeTextContent(node)) === target) {
                return node;
            }
        }
        return null;
    ...
    /** キャッシュ由来の空 user やサーバー確定済み user の文言欠落を補正 */
    function sanitizeSessionMessages(messages) {
        ...
        list.forEach(function (m) {
            ...
            if (m.type === 'user') {
                let text = userMessageText(m);
                if (!text && lastUser && (m.uuid || m.message_id)) {
                    m = Object.assign({}, m, { content: lastUser });
                    text = lastUser;
                }
                if (!text && !m.uuid && !m.message_id) {
                    return;
                }
            }
            out.push(m);
    ...
    function isRedundantLocalUserMessage(serverMsgs, localMsg) {
        if (!localMsg || localMsg.type !== 'user') {
            return false;
        }
        const localText = userMessageText(localMsg);
        if (!localText) {
            return true;
        }
        if (isRedundantOptimisticUser(serverMsgs, localMsg)) {
            return true;
        }
        ...
        return (serverMsgs || []).some(function (m) {
            ...
            return userMessagesMatch(m, localMsg);
        });
    }

    function messageNodeTextContent(node) {
        return (node && node.querySelector('.message-content')?.textContent || '').trim();
    }

    function messageNodeHasRicherContent(candidate, incumbent) {
        return messageNodeTextContent(candidate).length > messageNodeTextContent(incumbent).length;
    }

    function refreshUserMessageNodeContent(node, message) {
        if (!node || !message || message.type !== 'user') {
            return;
        }
        const text = userMessageText(message);
        if (!text) {
    ...
    function collapseDuplicateUserDomNodes(chatMessages, messages) {
        ...
        chatMessages.querySelectorAll('.message.user').forEach(function (node) {
            ...
            const text = comparableUserMessageText(messageNodeTextContent(node));
            if (!text) {
                prevUser = node;
                prevText = text;
                return;
    ...
    /** 直近ターンの user ノード（楽観表示・確定 uuid 両対応） */
    function findLastUserMessageDomNode(chatMessages, messages) {
        if (!chatMessages) {
            return null;
        }
    ...
    /** 直近 user より後にある bot ノード（処理バブル・初期メッセージ除く） */
    function getBotNodesAfterLastUserDom(chatMessages, messages) {
        const lastUserNode = findLastUserMessageDomNode(chatMessages, messages);
        if (!lastUserNode) {
            return [];
        }
        const bots = [];
        chatMessages.querySelectorAll(
            ...
            if (lastUserNode.compareDocumentPosition(node) & Node.DOCUMENT_POSITION_FOLLOWING) {
                bots.push(node);
            }
        });
        return bots;
    ...
    function botNodeTextMatchesMessage(botNode, botMsg) {
        if (!botNode || !botMsg) {
            return false;
        }
        ...
        const text = (botNode.textContent || '').replace(/\s+/g, ' ').trim();
        if (!text) {
            return false;
        }
    ...
    function isLatestUserMessageRendered(messages) {
        ...
        return botNodeTextMatchesMessage(turnBots[turnBots.length - 1], lastBotMsg);
    }
    ...
    function resetIdleChatStateOnPageLoad() {
        ...
        chatSubmitBaselineCount = 0;
        forceReleaseChatSubmitLock(getSidFromCookie());
        forceReleaseChatStreamLock(getSidFromCookie());
    }
    ...
    function dedupeMessageList(messages) {
        ...
        list.forEach(function(m) {
            ...
            if (m.type === 'user' && out.length > 0) {
                ...
                if (prev.type === 'user' && userMessagesMatch(prev, m)) {
                    ...
                    if ((m.uuid || m.message_id) && !(prev.uuid || prev.message_id)) {
                        seen.delete(stableMessageKey(prev));
                        out[out.length - 1] = m;
                        seen.add(k);
                        return;
        ...
        return collapseDuplicateUserMessages(stripRedundantOptimisticUsers(out));
    }
    ...
    function normalizeTurnTail(messages, botMsg, userMsg, lastUserText) {
        ...
        while (msgs.length > 0 && msgs[msgs.length - 1].type === 'user') {
            ...
            const isPending =
                !isServerConfirmed &&
                ((lastUser && userMessagesMatch(tailText, lastUser)) ||
        ...
        if (userCandidate) {
            ...
            const skipOptimisticDuplicate =
                lastUser
                && userMessagesMatch(candidateText, lastUser)
    ...
    function repairOrphanPendingUser(messages, lastUserText) {
        ...
        const tail = msgs[msgs.length - 1];
        if (tail.type !== 'user' || !userMessagesMatch(tail, lastUser)) {
            return msgs;
        }
    ...
    function mergeMessageLists(serverMsgs, localMsgs) {
        ...
        local.forEach(function(m) {
            if (isRedundantLocalUserMessage(server, m)) {
                return;
            }
            const k = stableMessageKey(m);
            if (!m || seen.has(k)) {
        ...
        return sanitizeSessionMessages(merged);
    }
    ...
    function resolveSessionMessages(sessionData, options) {
        const opts = options || {};
        const sid = (sessionData && sessionData.session_id) || getSidFromCookie();
        const server = sanitizeSessionMessages(
            (sessionData && sessionData.messages) ? sessionData.messages : []
        );
        if (isSessionResetPending()) {
            return server;
    ...
    function recoverCompletedInflightOnLoad(sessionData, merged) {
        const sid = getSidFromCookie();
        const inflight = loadInFlightTurn(sid);
        if (inflight && hasServerCompletedInFlightTurn(sessionData, inflight)) {
            clearInFlightTurn(sid);
            if (!applyBotResponseSession(sessionData, { preserveStatusCards: true, forceRender: true })) {
    ...
    function tryRestoreInFlightProcessingOnLoad(sessionData) {
        ...
        if (recoverCompletedInflightOnLoad(sessionData, merged)) {
            return;
        }
        const inflight = loadInFlightTurn(sid);
        ...
        fetch(withVersion('/api/processing-status'), {
            ...
            .then(function (data) {
                if (!data || !data.active) {
                    if (processingBubbleLocked && inflight) {
                        activeSubmitSid = sid;
                        scheduleDeferredSessionRecovery(chatSubmitGeneration, 0, {
                            maxAttempts: 30,
                            force: true,
    ...
    function restoreSubmitButton() {
        ...
        clearSubmitWatchdog();
        clearSlowRequestTimer();
        forceReleaseChatSubmitLock(resolveActiveSubmitSid());
        activeSubmitSid = '';
        if (submitBtn) {
    ...
    function notifySlowRequest(event) {
        ...
        if (typing) {
            ...
            const pill = typing.querySelector('.processing-status-step-pill');
            const fill = typing.querySelector('.processing-status-bar-fill');
            if (label && label.textContent) {
                clientContext.processing_label = label.textContent.trim();
            }
            if (pill && pill.textContent) {
                clientContext.processing_step = pill.textContent.trim();
            }
            if (fill && fill.style && fill.style.width) {
                clientContext.processing_percent = fill.style.width;
            }
        }
...
function appendQaDelta(text, section) {
    ...
    function hasActiveStreamingContent() {
        ...
        if (rec) {
            ...
            if (
                rec.querySelector('.medicine-item, .ui-reco-block, .ui-carousel, .ui-reco-carousel')
                || rec.classList.contains('has-medicines')
            ) {
                return true;
            }
        }
        ...
        const qaSections = document.querySelector('.streaming-qa-sections');
        if (qaSections && qaSections.children.length > 0) {
            return true;
        }
        return false;
    }

    /** 応答が画面に出ていないときは処理バブルを維持（force 時も同様） */
    function dismissTypingIndicatorWhenReady(messages, options) {
        const opts = options || {};
        if (opts.force && !postResponseResolved && !isResponseVisibleInDom(messages)) {
            return false;
        }
        scheduleTypingIndicatorRemoval(messages);
        return true;
    ...
    function tryPromoteStreamingRecommendation(message, index) {
        ...
        const wrapper =
            streamingRecommendationEl && streamingRecommendationEl.isConnected
                ? streamingRecommendationEl
                : document.querySelector('[data-streaming-recommendation="true"]');
        if (!wrapper) {
            return false;
        }
        const messageKey = getMessageDomKey(message);
    ...
    function finalizeStreamingUiAfterPost() {
        if (!postResponseResolved && !isResponseVisibleInDom(null)) {
            scheduleDeferredSessionRecovery(chatSubmitGeneration, 0);
            return;
        }
        endAwaitingPostResponse();
        dismissTypingIndicatorWhenReady(null);
        removeProcessingMessage();
        restoreSubmitButton();
        clearSlowRequestTimer();
    }
    ...
    function fetchMessagesAfterPost(data, onComplete) {
        ...
        const maxRetries = postMeta.instantRendered
            ? 0
            : (postMeta.from_sse_done ? (hasDoneBot ? 8 : 10) : 5);
            ...
            const timeoutId = setTimeout(function () {
                if (settled || seq !== fetchSeq || shouldSuppressPostFetchError()) {
                    if (shouldSuppressPostFetchError()) {
                        dismissTypingIndicatorWhenReady(null, { force: true });
                        restoreSubmitButton();
                        clearSlowRequestTimer();
            ...
            fetch(withVersion('/api/sessions'), {
                ...
                .then(function (sessionData) {
                    ...
                    if (mergedAfterPost.length > 0 && !shouldDeferSessionSync()) {
                        ...
                        mergedAfterPost.length > 0
                        && isCurrentTurnComplete(mergedAfterPost)
                        && !isSessionRenderStable(mergedAfterPost)
                    ) {
                        applySessionMessages(sessionData, {
                            preserveStatusCards: true,
                            forceRender: true,
                    ...
                    if (postMeta.message_count === 0 && (!sessionData.session_active || sessionData.messages_count === 0)) {
                        if (postResponseResolved || isResponseVisibleInDom(mergedAfterPost)) {
        ...
        function finalizeSsePost(donePayload) {
            ...
            try {
                ...
                if (instantRendered) {
                    ...
                    if (donePayload && donePayload.bot_message) {
                        if (!unlockPostResponseUI(donePayload)) {
                            requestAnimationFrame(function () {
                                if (!postResponseResolved && unlockPostResponseUI(donePayload)) {
                                    resetRecommendationSseBulkState();
                                }
                            });
                        } else {
            ...
            const postUnlockFallbackTimer = setTimeout(function () {
            ...
            fetchMessagesAfterPost(
                ...
                function () {
                    ...
                    clearTimeout(postUnlockFallbackTimer);
                    clearSubmitWatchdog();
                }
            );
        }
                ...
                if (data.status === 'ok') {
                    ...
                    .then(function (sessionData) {
                        if (sessionData && sessionData.session_id) {
                            rememberSid(sessionData.session_id);
                        }
                        applySessionMessages(sessionData, {
                            forceRender: true,
                            suppressTypingIndicator: true,
                    ...
                    .catch(function (error) {
                        console.error('薬剤師要請後のセッション同期エラー:', error);
                        loadMessages();
                    });
                } else {
    ...
    function syncChatMessageOrder(messages, cardsByAnchor, chatMessages) {
        ...
        chatMessages.querySelectorAll('.message[data-message-id]').forEach((node) => {
            ...
            const key = node.getAttribute('data-message-id');
            if (key) {
                const prev = existingNodes.get(key);
                if (!prev || messageNodeHasRicherContent(node, prev)) {
    ...
    function ensureSessionMessagesInDom(messages) {
        ...
        messages.forEach(function (message, index) {
            ...
            if (message.type === 'user') {
                const existingUser = findUserMessageNodeByComparableText(chatMessages, message);
                if (existingUser) {
        ...
        messages.forEach((message, index) => {
            ...
            if (message.type === 'user') {
                const existingUser = findDuplicateUserContentNode(chatMessages, message)
                    || findUserMessageNodeByComparableText(chatMessages, message);
                if (existingUser) {
                    if (existingUser.getAttribute('data-message-id') !== messageKey) {
                        existingUser.setAttribute('data-message-id', messageKey);
                        existingUser.removeAttribute('data-temporary');
